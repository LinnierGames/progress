...
struct DateFormatterToken: CustomStringConvertible, ExpressibleByStringLiteral {
  ...
  static func from(string: String) -> DateFormatterToken {
    return DateFormatterToken(stringLiteral: string)
  }
...
extension Date {
  ...
  ///   ```
  /// 
  /// - Parameter formatter: formatter: the order and presentation of how the readable
  ...
  func formattedStringWith(formatter: DateFormatterToken...) -> String {
    return self.formattedStringWith(formatter: formatter)
  }

  func formattedStringWith(stringFormatter: String, timeZone: TimeZone = TimeZone.current, locale: Locale = Locale.current) -> String {
  ...
  fileprivate func formattedStringWith(formatter: [DateFormatterToken]) -> String {
    var compiledFormatterString = ""
    for aToken in formatter {
      compiledFormatterString.append(aToken.description)
    }

    return self.formattedStringWith(stringFormatter: compiledFormatterString)
  }
}
